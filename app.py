...
if "authenticated" not in st.session_state:
    ...
    parol = st.text_input("Kirish paroli:", type="password", key="p_main")
    if st.button("Kirish", key="b_main"):
...
with st.sidebar:
    ...
    st.divider()
    menu = st.radio("Bo'limni tanlang:", ["🤖 AI Muloqot", "📊 Jurnal Monitoringi"], key="menu_nav")
    st.divider()
    st.info(f"💡 {random.choice(HIKMATLAR)}")

# --- 6. AI MULOQOT (XATOLIK TUZATILDI) ---
if menu == "🤖 AI Muloqot":
    ...
    # NOYOB KEY QO'SHILDI: key="chat_input_unique"
    if savol := st.chat_input("Ism yozing (masalan: JALILOVA yoki Dilfuza)...", key="chat_input_unique"):
        ...
        with st.chat_message("assistant"):
            ...
            # 2. Bazadan qidiruv (Case-insensitive)
            if not topildi and sheets_baza:
                for key, df in sheets_baza.items():
                    mask = df.apply(lambda row: row.astype(str).str.contains(savol, case=False, na=False).any(), axis=1)
                    res_df = df[mask]
                    if not res_df.empty:
                        st.success(f"🔍 '{savol}' bo'yicha ma'lumotlar:")
            ...
            if not topildi:
                ...
                except: st.error("AI hozirda javob bera olmayapti.")

# --- 7. MONITORING (TEGILMADI) ---
    ...
    j_fayl = st.file_uploader("Excel yuklang", type=['xlsx', 'xls', 'html'], key="m_up")
    if j_fayl:
        try:
            ...
            if len(df_j.columns) >= 6:
                ...
                if st.button("📢 Telegramga yuborish", key="m_tg"):
